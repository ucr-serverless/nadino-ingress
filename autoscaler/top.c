...
#define TOP_DIFF_P(member) (ptop[j].member - potop[j].member)
    ...
    for (i = 0; ; i++) {
        if (max_proc_id == -1) {
            ...
            if (i % title_line == 0) {
                // printf("|---------|---------|---------|---------------|\n");fflush(stdout);
                // printf("|%9s|%9s|%9s|%15s|\n", "idle", "sys", "usr", "loop");fflush(stdout);
                // printf("|---------|---------|---------|---------------|\n");fflush(stdout);
            }
            ...
            if (i) {
                ...
                printf("|%8.2f%%|%8.2f%%|%8.2f%%|%15lu|\n",
                    idle, sys, usr, TOP_DIFF(loops));fflush(stdout);
            }
        }else {
            /*
            ...
            if (i % (title_line / (max_proc_id - proc_id + 2)) == 0) {
                ...
                //     "proc_id", "idle", "sys", "usr", "loop");fflush(stdout);
                // printf("|---------|---------|---------|"
                    // "---------|---------------|\n");fflush(stdout);
            }
            ...
            for (j = proc_id; j <= max_proc_id; j++) {
                ...
                if (i) {
                    ...
                    printf("|%9d|%8.2f%%|%8.2f%%|%8.2f%%|%15lu|\n",
                        j, pidle, psys, pusr, ploops);fflush(stdout);
                    ...
                    if (j == max_proc_id) {
                        printf("|%9s|%8.2f%%|%8.2f%%|%8.2f%%|%15lu|\n",
                            "total", idle, sys, usr, loops);fflush(stdout);
                        // printf("|         |         |         |"
                        //     "         |               |\n");fflush(stdout);
                    }
                }
